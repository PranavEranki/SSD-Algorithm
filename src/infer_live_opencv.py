...
def update(session, io, labels, colors, capture, should_flip = False):
	...
	Arguments:
		...
		capture: OpenCV capture device.
		should_flip: Flag whether to flip the image from the capture device. Defaults to False.
	"""
	...
	frame_model = cv2.resize(frame, (300, 300), interpolation=cv2.INTER_NEAREST)
...
if __name__ == "__main__":
	...
	argument_list = ArgumentList(
		description = "Apply an exported model on live data like a video or a webcam using OpenCV."
	)
	...
	with tf.Session(graph=graph) as session:
		...
		frame_ii = 0
		while True:
			update(session, io, dataset.selected_labels, colors, capture, should_flip=is_webcam)
			frame_ii += 1

			if not is_webcam:
				if frame_ii == capture.get(cv2.CAP_PROP_FRAME_COUNT):
