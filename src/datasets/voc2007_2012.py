...
class VOC2007_2012(DatasetSource):
	"""Handle the union of Pascal VOC 2007 and 2012 dataset.
	"""
	...
	def convert(self, split_name):
		if split_name not in ["train", "validation", "trainval"]:
		...
		# convert the split from VOC 2007 dataset
		devkit_path = os.path.join(self.paths["interim_2007"], "VOCtrainval_06-Nov-2007", "VOCdevkit", "VOC2007")
		self.__convert_split(split_name, devkit_path, writer)

		# convert the split from VOC 2012 dataset
		devkit_path = os.path.join(self.paths["interim_2012"], "VOCtrainval_11-May-2012", "VOCdevkit", "VOC2012")
		self.__convert_split(split_name, devkit_path, writer)
	...
	def __convert_split(self, split_name, devkit_path, writer):
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(
		description = "PASCAL Visual Object Classes 2007 + 2012"
	)
	parser.add_argument(
		"--convert",
		type = str,
		choices = ["train", "validation", "trainval"],
	...
	if arguments.convert is None and arguments.rgb_mean is None:
		parser.print_help()
		exit()
